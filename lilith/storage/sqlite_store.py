...
This is a small, dependency-light persistence primitive used by multiple
subsystems (symbolic pipeline, memory leaves, etc.).

It deliberately keeps the schema simple: (scenario, label, vector).
Higher-level stores can layer payload tables on top.
"""
...
class SQLiteVectorStore:
    ...
    def add(self, embeddings: torch.Tensor, labels: torch.Tensor, *, scenario: Optional[str] = None) -> None:
        ...
        rows = []
        for label, vec in zip(labels_np, embeddings_np):
    ...
    def fetch(self, scenario: Optional[str] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        vectors_list = [np.frombuffer(row[1], dtype=np.float32).copy() for row in rows]

        # Defensive: databases may contain embeddings from older encoder configs.
        # Keep only the most common dimensionality so retrieval doesn't crash.
        lengths = [int(v.shape[0]) for v in vectors_list]
        target_dim = int(np.bincount(np.array(lengths, dtype=np.int64)).argmax())
        kept_vectors = [v for v in vectors_list if v.shape[0] == target_dim]
        kept_labels = [lab for lab, v in zip(labels.tolist(), vectors_list) if v.shape[0] == target_dim]
        if not kept_vectors:
            raise RuntimeError("Vector store contains no consistent vector shapes.")

        labels = torch.tensor(kept_labels, dtype=torch.long)
        vectors_np = np.stack(kept_vectors, axis=0)
